...
\begin{itemize}
    ...
    \item \textbf{Complejidad Temporal:} $O(\prod_{i=1}^n (q_i + 1))$. Debido a su crecimiento exponencial, este algoritmo solo es viable para instancias muy pequeñas ($n < 15$).
...
\begin{itemize}
    ...
    \item \textbf{Complejidad Temporal:} $O(n \cdot M \cdot E \cdot \max(q_i))$. Con $n=200, M=3000, E=500, q_i=30$, el total de operaciones es manejable en tiempos modernos ($\approx 9 \times 10^9$ operaciones teóricas, pero mucho menos en la práctica por las restricciones de recursos).
